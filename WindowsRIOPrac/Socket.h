...
public:
	TSocket( void ) = delete;
	TSocket( int type, int protocol, int flags );
	virtual ~TSocket( void );

	virtual HANDLE						getHandle( void ) const noexcept;
	virtual SOCKET						getSocket( void ) const noexcept;

	virtual std::string					getName( void ) const noexcept;
	virtual std::string					getPeerName( void ) const noexcept;
...
private:
	...
	TWSAStartUp							_wsaStartup;
	TWinsockExtensions*					_winsockExtensions;
